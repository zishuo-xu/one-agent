...
把工具数量作为运行时信号交给 `StrategyController`：单工具继续直接执行；首批出现多个工具时升级到
...
| 执行策略 | 已实现 | Simple、Planning、Auto Planning；Auto 可在首批多工具执行前安全升级一次 |
...
2. **Strategy Controller / AdaptiveStrategy**：V1 已实现首批多工具触发的安全 `simple→planning`，并写入 Trace、限制最多一次。根据工具反馈、不确定性和剩余预算继续选择策略仍属于候选方向。
