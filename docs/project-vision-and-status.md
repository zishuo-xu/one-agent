...
Agent 通过 `ModelProvider` 使用模型，并支持主备模型切换。Provider 除了归一请求与响应，还声明流式输出、
...
| 能力面 | 当前状态 | 实现边界 |
|---|---|---|
| 模型抽象 | 已实现 | Provider 能力契约、创建前硬要求校验、主备链能力交集、OpenAI 兼容适配与 utility model |
...
- 当前只有 OpenAI Compatible Adapter；能力契约已经允许新增其他协议 Provider，但尚未实现 Anthropic 等原生适配器；
