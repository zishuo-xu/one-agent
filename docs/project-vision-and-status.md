...
每个父 Run 在完成记忆召回后，将本轮已经筛选出的只读记忆快照交给 `SubAgentRunner`；Simple 临时委派与 Planning
计划委派因此使用同一上下文边界。子 Agent 仍不能直接查询或修改 Memory Store，新的父 Run 会先清除旧快照，
避免跨请求泄漏上一次召回结果。
