...
模型协议、Endpoint、Key、模型名及主备链统一维护在 `one-agent.config.json` 的 `model` 分组中。
...
RunContext（一次请求的 runId、signal、memory、reasoning、recovery）
      ...
      ├── ContextManager / MemoryDocumentStore
      └── RunRecorder / SQLite
...
包边界与运行边界一致：`agent-core` 只公开在线 Runtime 所需能力；`agent-eval` 依赖
`agent-core`，承载 EvalRunner、Completion Contract、确定性回放 Provider 和评测数据集。
Runtime 不依赖 `agent-eval`。`AgentRuntime.createAgent()` 在调用方未显式指定策略时统一读取
`runtime.loop`，因此 CLI、REST API 与 TaskQueue 使用同一默认 Loop，不再由各入口分别补配置。

`AgentLoop` 内部按一次 Run 的生命周期分为三个私有阶段，不增加新的服务层：

```text
prepareRun       → 请求上下文、Run 关联、起始 Trace
executeStrategy  → 审批继续、记忆文档加载、Loop 选择与执行
...
用户请求
  ...
  → 加载全局与工作空间记忆文档并记录 memory_context_loaded
  → 将两份文档转换为统一 Memory Context
...
默认策略允许普通工具，`delete_file` 和 `run_command` 需要用户确认；名单由统一配置表的
`tools.requireApproval` 提供，配置层只声明规则，不参与授权决策。Runtime 使用工具名称与规范化参数生成
...
同一次模型响应中的工具调用作为一个批次交给 `ToolRunner`。只有当批次中每个工具都在
`ToolDefinition.readOnly` 显式声明只读时，执行层才并发启动；混合批次、写入工具和未知工具保持原有串行语义。
执行前仍对整个批次完成 Policy 预检，避免某个需审批调用等待时，同批其他工具已经产生副作用。并发任务可以按实际
完成时间产生运行事件，但工具结果、模型上下文和持久化记录始终按模型原始调用顺序提交，因此 Trace 可稳定阅读和重放。
该规则不认识 `spawn_agent` 等具体工具名称，也不新增 Loop、状态或配置；SimpleLoop 只提交批次，PlanningLoop 继续使用
计划中的显式并行波次和逐步执行语义。

CLI 默认只向用户流式展示 `message_delta`，并原样保留 Provider 单独发送的空格和换行块，避免 Markdown
代码块、标题或列表在实时输出中粘连。模型 `reasoning_delta` 始终写入 Trace，只有显式启用
...
### 5.3 Plan Review：规划层的执行前控制点

交互式 PlanningLoop 生成计划后先冻结当前计划并返回 `plan_approval`，在用户确认前不执行任何步骤或工具。
用户可以批准执行、拒绝并以“零执行”结束，或提交一次修改意见；修改意见只交给 Planner 重新生成计划，
随后必须再次确认。第二次确认只接受批准或拒绝，避免把交互演变成复杂的计划编辑工作流。

Plan Review 只归规划决策层所有。它复用 AgentLoop 已有的 `waiting_for_input` 生命周期和 Trace 恢复事实，
但不新增 Loop、组件、数据库表或独立 Checkpoint。CLI/API 只负责展示与提交答案，SimpleLoop 不受影响，
非交互式 TaskQueue 自动跳过。计划批准后，高风险工具仍需由执行层 `ToolPolicy` 单独逐次授权；批准计划
不等于批准副作用。正常的确认、修改和工具审批续跑不消耗异常崩溃的三次恢复额度。开关统一配置为
`runtime.planApproval`，默认开启。

### 5.4 Sub-Agent：受限的只读执行单元
...
`executionStatus=failed / outcomeStatus=unavailable`。其压缩事件流和状态嵌入父 Run Trace，不创建独立 Thread 或 Run。

Evidence Packet 不通过第二次模型调用生成。Runtime 将子 Agent 的最终回答作为 `conclusion`，再从已发生的
`tool_call/tool_result` 事件确定性配对最多 8 条成功观察，记录 `toolCallId`、工具名、可识别的 path/URL 来源和最多
1200 字符的观察值；失败工具进入 `uncertainty`，未获得任何成功工具证据时明确标记为 model-only，并把未满足的
`expectedEvidence` 列入 `unresolvedQuestions`。这套协议提供来源与已知缺口，不把子 Agent 结论升级为已验证事实。
SimpleLoop 与 PlanningLoop 都把结构化 Packet 交给父 Agent；旧的 `summary` 字段只保留兼容，不再是父 Agent 的唯一输入。
...
- `plan_review`：计划确认的请求、批准、拒绝和一次修改意见；
...
- `memory_context_loaded`：已加载作用域、文档 hash、字符数和上下文成本；
...
仍按 `trace.contentMode` 执行脱敏；默认值为 `redacted`，也支持 `metadata` 和 `full`。
...
PlanningLoop 的 `plan_approval` 也复用这套协议：计划及修改次数位于恢复点，重新打开 Thread 后继续显示。
批准、拒绝或一次修改反馈都创建新 Run 并与旧 Run 关联；拒绝不会执行步骤，修改后再次进入等待。
它不改变澄清和工具审批的职责，也没有新增持久化结构。
...
- 显式管理直接追加、精确替换或删除 Markdown 文本，不额外调用提取模型；Sub-Agent 不继承记忆管理权限；
- 全局和工作空间更新共用一把本地独占锁，允许低频写入等待；文件使用临时文件和原子替换，读取永远看到完整版本；
- 外部编辑器不遵守运行时锁，因此写回前再次比较内容 hash；发生变化时放弃写入并保留 Thread 未提取状态，绝不覆盖用户编辑；
- 文件先提交、Thread 后标记；中间崩溃只会触发基于最新文档的幂等重试；
- `manage_memory` 拒绝保存密码、API Key、Token、Secret 等凭据型记忆；
- 每轮记录 `memory_context_loaded` Trace，只保存作用域、文档 hash、字符数和估算 token，不复制文档内容；
- 每轮重新读取两份文档并重建 Memory Context；当前输入优先，记忆内容不构成指令或工具授权；
- 主 Agent、Planner 与 Sub-Agent 复用同一份已格式化记忆快照，不各自维护记忆提示词；
- CLI 可查看两份完整文档；API 和 Trace Web 提供全局/工作空间文档的读取与带 hash 冲突保护的编辑入口。

SQLite 不再保存长期记忆内容，也没有候选表、版本链、置信度、过期状态或墓碑。未来 RAG 以两份 Markdown 为原始文档，索引只作为可重建派生数据，不成为第二事实源。
旧数据库升级时如存在 `memories`，只重命名为运行时永不读取的 `memories_legacy` 回滚档案；新数据库不会创建该表。
...
- 模型中途询问仍只处理关键澄清；计划确认和危险工具审批由各自所有者发起，三者都不支持多人审批或超时策略；
- Plan Review 当前只支持整份计划批准、拒绝和一次自然语言修改意见，不提供逐步骤编辑、合并或多人会签；
- Plan Review v1 确认初始计划或用户修改后的计划；执行中因失败触发的预算内自动重规划不会再次暂停，但新出现的高风险工具仍需独立审批；
...
4. 继续消除 `tasks.events`、`reasoning_chain` 等剩余重复状态来源，强化数据库迁移和状态约束；
