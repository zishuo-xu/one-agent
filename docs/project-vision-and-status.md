...
RunContext（一次请求的 runId、signal、memory、reasoning、recovery）
      ...
      ├── ToolRunner ── ToolExecutor / ToolRegistry / Sandbox
...
`ToolRunner` 负责统一工具执行协议，`RunRecorder` 负责事实记录。`RunContext` 只是随一次请求传递的数据对象，
...
用户请求
  ...
  → 缺少关键输入：记录问题、Checkpoint 和 waiting_for_input
  → 刷新 Trace 缓冲区
  → 立即向用户返回答案或待回答问题
```
...
并把问题与恢复位置写入现有 `agent_runs.checkpoint`；当前 Run 随即结束执行并返回用户。CLI 关闭后，
...
这一能力没有新增数据库表：`agent_runs.status` 增加 `waiting_for_input`，Checkpoint 使用
`loopMode` 区分 SimpleLoop 与 PlanningLoop。Trace 仍只记录事实，不会在询问后触发自动验证或优化。
...
| 运行中工作状态 | 部分实现 | RunContext 已统一一次请求的关联信息；Plan、ReasoningChain、Checkpoint 各自保留明确职责，尚无语义级 WorkingState |
...
- 中途询问 v1 只处理关键澄清，不处理危险工具审批、多人审批或超时策略；
