...
> 文档状态：历史工程记录（非当前 CLI 手册）
> 阅读说明：本文按提交发生时保留交互设计、测试数量与限制。当前使用方式见 [根 README](../README.md)，历史分类见 [文档索引](./README.md)。
...
- 首个文本到达后记录 **首字耗时**（TTFA）。
- 结束显示拆分为：`(首字 1.2s · 回答 6.8s · 工具 0.4s · 记忆：切换或退出会话时整理)`。
- 工具调用阶段记录工具阶段耗时。
- `AgentLoop` 不再逐轮提取；切换 Thread 时后台整理离开的会话，退出时整理当前会话。
- 记忆状态仅 `--verbose` 显示，避免误导用户认为每轮都额外调用模型。
