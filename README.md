...
长期记忆继续使用单张 `memories` 表，不引入向量数据库或自动优化器。每条记忆记录来源 Thread/Run、
`global`/`thread` 作用域、置信度、生命周期状态、过期时间和最近召回时间。召回只使用有效且未过期的记忆；
同一作用域的同名事实发生冲突时，置信度不低于旧事实的新值会替代旧值，低置信度冲突会保留为历史记录但不参与召回。
模型异步抽取的事实默认是 `global`、置信度 `0.7`，所以仍支持跨会话使用，同时能够追溯到产生它的 Run。
