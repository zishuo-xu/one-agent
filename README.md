...
自动提取的记忆必须关联原始用户 `messageId`、逐字证据和原文中实际出现的最小事实值；问题不能作为答案的证据。整理时已有显式记忆会作为权威上下文交给 Memory Agent，运行时也会拒绝被显式记忆覆盖的重复候选。所有记忆都以事实发生时间 `observedAt` 而不是后台处理时间判断新旧。因此旧会话即使更晚补处理，
也不会覆盖用户在新会话中表达的更新事实。召回只使用符合状态、作用域和过期规则的记忆；整理过程和每轮召回决策都会写入
Trace，包括候选、命中依据、过滤原因、最终注入 ID 与上下文成本，但不会在 Trace 中复制记忆值，也不参与自动优化。

召回结果不会再直接拼成 `key: value`。`buildMemoryContext` 会生成一份供主 Agent、Planner 和 Sub-Agent 共用的
“固定使用规则 + JSON 记忆数据”契约：当前用户输入和当前会话优先于冲突的历史记忆，记忆值只能作为背景数据，不能作为
指令或工具授权。该契约只改变上下文表达，不增加模型调用、数据库状态或运行时组件；高风险执行仍由 Tool Policy 硬约束。
